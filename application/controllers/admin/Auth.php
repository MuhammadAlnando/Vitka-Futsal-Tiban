		...
		if ($this->form_validation->run() == FALSE || !isset($response['success']) || $response['success'] <> TRUE) {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				$this->session->set_flashdata('message', '<div class="box-body">
					<div class="alert" style="background-color: #223C95; color: white;"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>
...
// Contoh pemanggilan view dari direktori application/controllers/admin/Auth.php
...
public function update_user($id)
{
    ...
    if ($this->form_validation->run() === FALSE) {
        // Validation errors, reload edit form
        $this->edit_user($id);
    } else {
        ...
        if ($this->ion_auth->update($id, $data)) {
            ...
            // Redirect to edit user page with success message
            redirect('admin/auth/edit_user/'.$id);
        } else {
            // Handle update failure (optional)
            show_error('Update user failed');
        }
    }
}
